...
RAW=${OPSD_ROOT}/data/raw/countdown_star_fullthink_all.parquet
TOKENIZED=${OPSD_ROOT}/data/sft/countdown_star_fullthinkALL_Qwen3-4B
CKPT=${OPSD_ROOT}/checkpoints/sft_star__m_Qwen3-4B__d_cd_fullthinkALL__bs64__lr5e-6__ep3
...
if [ ! -f "$RAW" ]; then
    "$OPSD_ROOT"/envs/train/.venv/bin/python rl/build_star_sft_fullthink.py \
        --train data/raw/countdown_opsd_train.parquet --rollouts pilot/star_rollouts_shard0.jsonl pilot/star_rollouts_shard1.jsonl pilot/star_rollouts_shard2.jsonl --qid_offset 0 0 0 --out "$RAW" || exit 1
fi
...
SUBMIT="sbatch --partition=pli-c --gres=gpu:h100:4 --time=06:00:00 \
    --job-name=opsd_armD3_fullthinkALL \
    --output=${OPSD_ROOT}/logs/%x-%A.out \
    --export=ALL,PROJECT_ROOT=${DPT},BASE_MODEL=${BASE_MODEL},BASE_TOKENIZER=${BASE_MODEL},DATASET_NAME=cd_fullthinkALL,TRAIN_PATH=${TOKENIZED},CHECKPOINT_PATH=${CKPT},SFT_LR=5e-6,SFT_EPOCHS=3,SFT_MICRO_BATCH_SIZE=1,SFT_GRADIENT_ACCUMULATION_STEPS=16,SFT_MAX_LENGTH=16384,EXTRA_ARGS=--use_load_from_disk=True\ --report_to=none \
