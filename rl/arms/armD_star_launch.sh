...
RAW=${OPSD_ROOT}/data/raw/countdown_star_sft.parquet
TOKENIZED=${OPSD_ROOT}/data/sft/countdown_star_sft_Qwen3-4B
CKPT=${OPSD_ROOT}/checkpoints/sft_star__m_Qwen3-4B__d_cd_xplus__bs64__lr5e-6__ep3

# --- 1. Build the (prompt, response) parquet from the shared train split -----
if [ ! -f "$RAW" ]; then
    "$OPSD_ROOT"/envs/train/.venv/bin/python rl/build_star_sft_dataset.py \
        --train data/raw/countdown_opsd_train.parquet --out "$RAW" || exit 1
fi
...
SUBMIT="sbatch --partition=pli-c --gres=gpu:h100:4 --time=01:00:00 \
    --job-name=opsd_armD_star_sft \
    --output=${OPSD_ROOT}/logs/%x-%A.out \
    --export=ALL,PROJECT_ROOT=${DPT},BASE_MODEL=${BASE_MODEL},BASE_TOKENIZER=${BASE_MODEL},DATASET_NAME=cd_xplus,TRAIN_PATH=${TOKENIZED},CHECKPOINT_PATH=${CKPT},SFT_LR=5e-6,SFT_EPOCHS=3,SFT_MICRO_BATCH_SIZE=1,SFT_GRADIENT_ACCUMULATION_STEPS=16,SFT_MAX_LENGTH=4096,EXTRA_ARGS=--use_load_from_disk=True\ --report_to=none \
